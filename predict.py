...
def main(args):
    ...
    # load model config 
    with open('./ProMIAF.yaml', 'r', encoding='UTF-8') as f:
    ...
    model = ProMIAF(seq_feature_matrix.shape[1],text_feature_matrix.shape[1],struct_feature_matrix.shape[1], model_config['n_hidden'], label_matrix.shape[1],
    ...
    model.load_state_dict(torch.load(os.path.join(args.model_dir, F'ProMIAF_{args.ontology}_{args.model_id}.ckp'))) 
    ...
    if args.input_file == None:
        with open(os.path.join(result_dir, F'{ont}_prediction_ProMIAF_{args.model_id}.txt'), 'w') as f:
        ...
        with open(os.path.join(result_dir, args.input_file+f'_ProMIAF_prediction.txt'), 'w') as f:
            for i in range(pred_combine_score.shape[0]):
                for j in range(pred_combine_score.shape[1]):
                    f.write(F'{tmp_pid_list[i]}\t{label_classes[j]}\t{pred_combine_score[i, j]}\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gpu", type=int, default=0,
                        help="gpu")
    parser.add_argument("--model_dir", type=str, default='./ProMIAF_models',
    ...
    logger.info("Running the ProMIAF model for prediction.")
